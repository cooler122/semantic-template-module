...
public class LackParamMatchComponentImpl extends FunctionComponentBase<List<SentenceVector>, Object> {
    ...
    protected ComponentBizResult<Object> runBiz(ContextOwner contextOwner, List<SentenceVector> sentenceVectors) {
        ...
        if(canceledLPMContextIdData != null){
            ...
            for (Integer canceledLPMContextId : canceledLPMContextIdSet) {                                              //太远的记录就删掉算了
                int contextDistance = currentContextId - canceledLPMContextId;
                if(contextDistance > 10 && contextDistance < -10){
                    canceledLPMContextIdSet.remove(canceledLPMContextId);
                }
            }
        ...
        Map<Integer, SVRuleInfo> historySVRuleInfoMap = null;
        for (DataComponent<SVRuleInfo> historyDataComponent : historyDataComponents) {
            if(historyDataComponent != null && historyDataComponent.getData() != null){
                ...
                if(!canceledLPMContextIdSet.contains(historyContextId)){                                                //上下文ID需要不在缺参排除列表中
                    ...
                    if(calculationLogType != Constant.NO_CALCULATION_LOG){
                        if(historySVRuleInfoMap == null) {
                            historySVRuleInfoMap = new HashMap<>();
                        }
                        String historySVRuleInfoDataJS = JSON.toJSONString(historySVRuleInfoData);                      //使用阿里的fastJson做深克隆，哎无奈之举啊！
                        SVRuleInfo cloneSVRuleInfo = JSONObject.parseObject(historySVRuleInfoDataJS, SVRuleInfo.class);
                        historySVRuleInfoMap.put(historyContextId, cloneSVRuleInfo);
                    }
                }
            }
        }
        ...
        for (SentenceVector sentenceVector : sentenceVectors) {
            ...
            Map<String, WordInfo> redundantWordInfoMap = new HashMap<>();                                               //多余词语记录集合
            for (List<REntityWordInfo> rEntityWordInfos : rEntityWordInfosList) {
                for (REntityWordInfo rEntityWordInfo : rEntityWordInfos) {
                    ...
                    redundantWordInfoMap.put(word, redundantWordInfo);
            ...
            for (Integer historyContextId : historyDataComponentMap.keySet()) {
                ...
                if(historyData != null) {
                    ...
                    if(historyLackedRRuleEntities != null && historyLackedRRuleEntities.size() > 0){                   //看看当前选定的上下文是否有缺失参数，有的话才进行缺参匹配
                        ...
                        Map<String, WordInfo> cloneRedundantWordInfoMap = new HashMap<>();
                        cloneRedundantWordInfoMap.putAll(redundantWordInfoMap);                                         //每一轮循环要将多余词语集的Map克隆一份
                        ...
                        for(RRuleEntity historyLackedRRuleEntity : historyLackedRRuleEntities){                         //遍历缺失实体集合
                            ...
                            if(rEntityWordInfo != null) {                                                                //不为空，则表示查找成功，尝试合并
                                List<Double> weights = rEntityWordInfo.getWeights();                        //当前句子向量中REW的weight
                                Double sv_weight = weights.get(sentenceVectorId);                                     //获取当前句子向量的此REW的句子端weight
                                ...
                                cloneRedundantWordInfoMap.remove(rEntityWordInfo.getWord());                                 //匹配上了的词语，就不是多余的词语了
                        ...
                        List currentRedundantWordInfos = new ArrayList();
                        currentRedundantWordInfos.addAll(cloneRedundantWordInfoMap.values());                           //准备好一个多余词语集合，设置到本轮产生的最优SVRuleInfo中
                        ...
                        if(calculationLogType == Constant.NO_CALCULATION_LOG){                                      //如果不需要打印日志，那么就判断一下发现不符合就直接跳出去；而需要打印日志的话，就需要看日志细节，那么就要记录评判过程，不要跳出去
                        ...
                        for (REntityWordInfo historyMatchedREntityWordInfo : historyMatchedREntityWordInfos) {          //此historyMatchedREntityWordInfos在后面的缺参相似度计算中将要使用，在这个循环中将为其每一个REW重新规制权重
                            ...
                            List<Double> historyMatchedREntityWeights = historyMatchedREntityWordInfo.getWeights();     //REW的权重Map<sentenceVectorId, weight>
                            ...
                            if(rewContextId == currentContextId){                                                              //如果此REW版本等于当前对话版本，那么它就是本轮句子产生的REW（它应该来源于本轮对话的句子向量sv）
                                ...
                                List<Double> weights = currREntityWordInfo.getWeights();                    //取出其权重Map
                                Double rewWeight = weights.get(sentenceVectorId);                                     //找出针对当前句子向量的此REW的权重

                                historyMatchedREntityWeights.set(sentenceVectorId, rewWeight * (1 - (amnesiacCoefficient * totalUsedHistoryRuleWeights)) / totalUsedCurrSVInfoWeights);   //从新为此REW设置针对当前句子向量的权重值（此有公式）
                            ...
                            //2.2.6.3.如果这个REW是历史对话中存储的REW，按公式设置它权重
                                ...
                                historyMatchedREntityWeights.set(sentenceVectorId, historyMatchedRRuleEntityWeight * amnesiacCoefficient);        //从新为此REW设置针对当前句子向量的权重值（此有公式）
                        ...
                        if(calculationLogType != Constant.NO_CALCULATION_LOG){
                            ...
                            similarityCalculationDataLPM.setSvInputREWIs(historyMatchedREntityWordInfos);
                            similarityCalculationDataLPM.setRuleMatchedRREs(historyMatchedRRuleEntities);
                            similarityCalculationDataLPM.setRuleLackedRREs(historyLackedRRuleEntities);
                        ...
                        resultSvRuleInfo.setRedundantWordInfos(currentRedundantWordInfos);                                     //设置本轮产生的多余词语集合
//                        System.out.println(algorithmType + "\n" + JSON.toJSONString(historySvRuleInfo) + "\n" + JSON.toJSONString(historyMatchedRRuleEntityMap) + "\n --- > \n" + JSON.toJSONString(resultSvRuleInfo));
                        ...
                        if(calculationLogType != Constant.NO_CALCULATION_LOG){
                            ...
                            coupleAlterationRateData.setIsCrossed(isCrossed);                                                 //*****************缺参日志准备：2-9
...
//            Integer finalSentenceVectorId = lpm_optimalSvRuleInfo.getSentenceVectorId();
...
//                Double finalWeight4REWI = matchedREntityWordInfo.getWeights().get(finalSentenceVectorId);
//                matchedREntityWordInfo.setWeights(Arrays.asList(finalWeight4REWI));                                     //一旦选好了向量ID，就将此ID下的权重设置给此REWI
            ...
            lpm_optimalSvRuleInfo.setSentenceModified(wordSB.toString());
